...
app.post("/catalog", authenticate, async (req, res) => {
  const { product_name, price, quantity, supplier, description, sku } =
    req.body;
  if (!product_name || !price || !quantity || !sku) {
    return res
      .status(400)
      .json({ message: "Product name, price, quantity, and sku are required" });
  }
...
app.delete("/catalog/:productId", authenticate, async (req, res) => {
  ...
  try {
    const product = await db.Models.Inventory.findOne({
      _id: productId,
      username: req.user.username,
    });
    if (!product) {
